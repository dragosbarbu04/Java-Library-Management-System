        ...
        try (Scanner sc = new Scanner(new File(RESOURCE))) {
            while (sc.hasNextLine()) {
                ...
                try {
                    ...
                    boolean removed = Boolean.parseBoolean(properties[4]);
                    Patron patron = new Patron(id, name, phone, email, removed);
                    library.addPatron(patron);
                } catch (NumberFormatException ex) {
                    throw new LibraryException("Unable to parse patron id " + properties[0] + "\nError: " + ex);
                }
            }
        }
    }
    ...
    public void storeData(Library library) throws IOException {
        try (PrintWriter out = new PrintWriter(new FileWriter(RESOURCE))) {
            for (Patron patron : library.getPatrons()) {
                ...
                out.print(patron.isRemoved() + SEPARATOR);
                out.println();
            }
        }
    }
